...
for v in range(len(varnames)):
    ...
    diff_FITHIT = np.nanmean(tas_dj[1] - tas_dj[0],axis=0)
    diff_FITCIT = np.nanmean(tas_dj[1] - tas_dj[2],axis=0)
    diff_HITCIT = np.nanmean(tas_dj[0] - tas_dj[2],axis=0)
    diff_FICCIT = np.nanmean(tas_dj[3] - tas_dj[2],axis=0)
    diff_FICTFIT = np.nanmean(tas_dj[4] - tas_dj[1],axis=0)
    diff_FICTHIT = np.nanmean(tas_dj[4] - tas_dj[0],axis=0)
    diffruns_dj = np.asarray([diff_FITHIT,diff_FITCIT,diff_HITCIT,diff_FICCIT,
                              diff_FICTFIT,diff_FICTHIT])
    
    ### Calculate significance for FM
    stat_FITHIT,pvalue_FITHIT = UT.calc_indttest(tas_dj[1],tas_dj[0])
    stat_FITCIT,pvalue_FITCIT = UT.calc_indttest(tas_dj[1],tas_dj[2])
    stat_HITCIT,pvalue_HITCIT = UT.calc_indttest(tas_dj[0],tas_dj[2])
    stat_FICCIT,pvalue_FICCIT = UT.calc_indttest(tas_dj[3],tas_dj[2])
    stat_FICTFIT,pvalue_FICTFIT = UT.calc_indttest(tas_dj[4],tas_dj[1])
    stat_FICTHIT,pvalue_FICTHIT = UT.calc_indttest(tas_dj[4],tas_dj[0])
    pruns_dj = np.asarray([pvalue_FITHIT,pvalue_FITCIT,pvalue_HITCIT,pvalue_FICCIT,
    ...
    fig = plt.figure()
    for i in range(len(diffruns_dj)):
        var = diffruns_dj[i]
        pvar = pruns_dj[i]
    ...
    plt.savefig(directoryfigure + 'allExperiments_DJ_%s.png' % varnames[v],
                dpi=300)
