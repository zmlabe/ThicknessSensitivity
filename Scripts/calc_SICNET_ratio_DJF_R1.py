"""
Compute ratio (%) between SIC and NET responses for DJF
...
for v in range(len(varnames)):
    ...
    experiments = [r'\textbf{FIC--CIT}',r'\textbf{FICT--HIT}']
...
#### Save file
np.savetxt(directorydata2 + 'sicNETratio_DJF_R1area.txt',np.round(meanratiovar.transpose(),1),delimiter=',',
           fmt='%3.1f',header='  '.join(varnames)+'\n',
           footer='\n File contains ratio values of relative contributions' \
           '\n between FIC-CIT and FICT-HIT to get the relative \n' \
...
labels = list(map(str,np.arange(0,101,100)))
...
plt.savefig(directoryfigure + 'SICNET_ratio_mesh_DJFarea.png',dpi=300)